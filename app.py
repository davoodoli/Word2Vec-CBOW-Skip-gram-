...
user_input = st.text_input('Enter a word from your text...')
if user_input:
    if st.session_state.model:
        try:
            similar_words = st.session_state.model.wv.most_similar(user_input)[:3]
            st.write("Words similar to '{}':".format(user_input), similar_words)
        except KeyError:
